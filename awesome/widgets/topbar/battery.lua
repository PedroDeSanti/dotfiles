...
local WIDGET_DIR = HOME .. '/.config/awesome/widgets/topbar/icons'
...
local function worker(user_args)
        ...
        awful.spawn.easy_async([[bash -c 'acpi']],
            function(stdout, _, _, _)
                ...
                notification = naughty.notify {
                    text = stdout,
                    title = '<span font="Ubuntu Nerd Font Bold 10" color="#a9b1d6">Battery status </span>',
                    timeout = 5,
                    height = 60,
                    width = 120,
    ...
    -- Apply margins if specified (like in Awful-DOTS)
    if margins then
        return wibox.container.margin(batteryarc_widget, margins, margins, margins, margins)
    else
